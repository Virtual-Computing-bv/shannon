import type { ScopeRule, ScopeRulePolicy, Settings } from '../shared/types.js';
...
export function createScopeRule(input: ScopeRuleInput): ScopeRule {
  ...
  const row = db.prepare(`SELECT * FROM scope_rules WHERE id=?`).get(Number(info.lastInsertRowid)) as unknown as ScopeRuleRow;
